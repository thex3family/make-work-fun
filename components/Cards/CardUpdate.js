...
export default function CardUpdate({ date, title, desc, desc2, img_url, button_url, version, button_class, loom_url, latest }) {
  return (
    <>
      {/* left card start */}
      ...
      <div className={`sm:col-start-1 col-span-3 col-end-4 my-4 mb-12 p-8 bg-primary-2 rounded w-full z-10 ${version % 2 ? 'md:col-start-1 md:col-end-3' : 'md:col-start-2 md:col-end-4'}`}>
        ...
        {loom_url ? (
          <iframe
            ...
            allow="accelerometer; 
                    ...
                    picture-in-picture"
          ></iframe>
        ) : <a className={button_class}><Link href={button_url}><img className="mb-4 m-auto cursor-pointer rounded-lg" src={img_url ? "/updates/" + img_url : null} /></Link></a>}
        ...
        <p className="text-sm font-semibold text-right mr-4">v.{version}</p>
      </div>
      {/* left card end */}
    </>
  );
