...
export default function CardPartyRecruit(props) {

    const {party, avatar_urls} = props;
    const style = {
    ...
    return(
        ...
        <div className={style.card}>
            ...
            <div className={style.description}>
                {exampleDescription + exampleDescription.length}
            </div>
            <div className={style.team}>
                <div className="flex">
                    {playersExample.map((player, i) => <img
                        src={player}
                        className={`${style.playerIcons} ${i > 0 ? style.member : style.leader}`}
                    ></img>)}
                </div>
                <div>
                    <Button
