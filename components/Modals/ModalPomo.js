...
export default function ModalPomo({ visibility, setVisibility, mobileDevice }) {
      ...
      <div className="fixed z-50 w-full h-0 px-5">
        <Draggable
          ...
          <div className={`${mobileDevice ? 'opacity-100' : 'opacity-20'} hover:opacity-100 inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl sm:my-8 sm:align-middle ${size == 'large' ? 'w-1/2' : size == 'medium' ? 'w-1/3' : 'w-full md:w-1/4'}`}>
            ...
            <div className="handle bg-gray-50 px-4 py-3 flex flex-row-reverse cursor-move">
              ...
              <button onClick={() => setVisibility(false)} type="button" className="inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 ml-3 w-auto text-sm">
                Close
              </button>
               </div>
            </div>
