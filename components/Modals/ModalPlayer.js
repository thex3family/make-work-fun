      ...
      <div className="fixed z-50 w-full h-0 px-5">
        <Draggable
          ...
          <div className={`${mobileDevice ? 'opacity-100' : 'opacity-100'} hover:opacity-100 inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl sm:my-8 sm:align-middle ${size == 'large' ? 'w-1/2' : size == 'medium' ? 'w-1/3' : 'w-full md:w-1/4'}`}>
            ...
            <iframe className="w-full" src={`/embed/player-details?auth=${playerURL}`} width="100%" height={650} frameBorder="0" allowFullScreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"></iframe>
